    ...
    <footer className="bg-background border-t border-border py-[40px]">
      <div className="container grid grid-cols-1 text-center md:text-start md:grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8">
        <div className="flex flex-col gap-4 md:gap-3">
          <h2 className="text-2xl font-semibold">Partify E-commerce</h2>
          ...
          <p className="text-muted-foreground text-start leading-relaxed max-w-full">
            At Partify, we're passionate about cars and committed to providing
            top-quality automotive products.
          </p>
        </div>
          ...
          {links.map((item) => {
            return (
              <Link
                ...
                key={item.id}
                target="_blank"
                className="duration-300 flex items-center justify-start hover:ps-2 gap-2 pb-3 w-full border-b border-border cursor-pointer">
                <ArrowRightCircle size={20} className="text-primary mt-[2px]" />
                {item.title}
              </Link>
            );
          })}
